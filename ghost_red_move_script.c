...
static void ghost_red_move_script_FREEDOM(Ghost* ghost, Map* M, const Pacman * const pacman);
...
static void ghost_red_move_script_FREEDOM(Ghost* ghost, Map* M, const Pacman * const pacman) {
    ...
    ghost->objData.nextTryMove = shortest_path_direc(M, ghost->objData.Coord.x, ghost->objData.Coord.y, pacman->objData.Coord.x, pacman->objData.Coord.y);
    /*
	static Directions proba[4]; // possible movement
	int cnt = 0;
	for (Directions i = 1; i <= 4; i++)
		if (ghost_movable(ghost, M, 3, false)) 	proba[cnt++] = i;
     */
