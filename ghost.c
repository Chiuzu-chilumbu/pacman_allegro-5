...
void ghost_draw(Ghost* ghost) {
	...
	*/
	...
	if (ghost->status == FLEE) {
		...
		{
            case RIGHT:
                al_draw_scaled_bitmap(ghost->dead_sprite, spriteSize * 0, 0, 16, 16,
                    drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
                    draw_region, draw_region, 0);
                break;
            case LEFT:
                al_draw_scaled_bitmap(ghost->dead_sprite, spriteSize * 1, 0, 16, 16,
                    drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
                    draw_region, draw_region, 0);
                break;
            case UP:
                al_draw_scaled_bitmap(ghost->dead_sprite, spriteSize * 2, 0, 16, 16,
                    drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
                    draw_region, draw_region, 0);
                break;
            case DOWN:
                al_draw_scaled_bitmap(ghost->dead_sprite, spriteSize * 3, 0, 16, 16,
                    drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
                    draw_region, draw_region, 0);
                break;
            default:
		...
		switch (ghost->objData.facing)
		{
            case RIGHT:
                sx = 0;
                al_draw_scaled_bitmap(ghost->move_sprite, spriteSize * sx, 0, 16, 16,
                    drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
                    draw_region, draw_region, 0);
                break;
            case LEFT:
...
bool ghost_movable(Ghost* ghost, Map* M, Directions targetDirec, bool room) {
	...
	int index_GX = ghost->objData.Coord.x, index_GY = ghost->objData.Coord.y;

    switch (targetDirec)
    {
        case 1:
            if (is_wall_block(M, index_GX, index_GY -= 1) || room == true )//|| is_room_block(M, index_GX, index_GY -= 1))
                    return false;
                break;
        ...
        case 2:
                if (is_wall_block(M, index_GX -= 1, index_GY) || room == true )//|| is_room_block(M, index_GX -= 1, index_GY))
                    return false;
                break;
...
void ghost_toggle_FLEE(Ghost* ghost, bool setFLEE) {
	...
	// You are allowed to do your own implementation of power bean system.
		...
		if(setFLEE){
			...
			if(ghost->status == FLEE)
                ghost->status = GO_OUT;
                ghost->speed = 2; //make speed slower
		}
}
...
void ghost_move_script_FLEE(Ghost* ghost, Map* M, const Pacman * const pacman) {
    ...
    //ghost_movable(ghost, M, shortestDirection, true);
    static Directions proba[4]; // possible movement
    int cnt = 0;
    for (Directions i = 1; i <= 4; i++)
        if (ghost_movable(ghost, M, 3, false)){
            proba[cnt++] = i;
        }
